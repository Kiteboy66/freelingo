...
Monorepo: `backend/` (Python 3.14 FastAPI) + `frontend/` (Next.js 16 App Router), deployable through Docker Compose or the three-service Railway profile. The free-first Railway path uses Neon-compatible PostgreSQL, Upstash-compatible Redis, Gemini for LLM work, Groq Whisper for STT, and a TTS-only Simba isiXhosa speech service with persistent model and MP3 caches. The backend proxies external services — the frontend never calls them directly except for its authenticated WebSocket connection to the public backend domain.

## Key constraints

- **Users can learn multiple languages simultaneously** — each language gets an isolated study plan, progress, flashcards, conversations, and competencies. Supported target languages: `en-US`, `en-GB`, `es-ES`, `it-IT`, `pt-PT`, `de-DE`, `fr-FR`, `ja-JP`, `ko-KR`, `zh-CN`, and A1-only `xh-ZA`. User's native language (asked at registration) is used for flashcard translations, tutor feedback, lesson and exercise `native_explanation` content, and cached native-language help in static grammar, phrasebook, and vocabulary resources. Study plans and assessments must expose only CEFR levels backed by real content for the selected language.
...
- **Free-first Railway uses three services** named `frontend`, `backend`, and `xhosa-speech`. The backend and speech service communicate over Railway private domains; the public backend domain is baked into `NEXT_PUBLIC_API_URL` for browser WebSocket sessions. Persist backend data at `/data` and Hugging Face models at `/models/huggingface`.
...
- `specs/architecture-backend.instructions.md` — Backend architecture: models (21), services (20), routers (23), schemas (15), env vars (75), Python code standards
...
- `specs/services.instructions.md` — **21 backend services**: LLM, TTS/STT, study plan, lessons, static-resource native help, flashcards, listening, reading, reviews, memory, progress, quotas, subscriptions, freemium, post-assessment voice trial, voice conversation pipeline
...
- `STT_PROVIDER=groq` → Groq's OpenAI-compatible transcription API (`GROQ_API_KEY` required)
- `STT_PROVIDER=openai` → OpenAI Whisper API (`OPENAI_API_KEY` required, no local service needed)

For local `xh` requests, the backend routes both speech paths to `XHOSA_SPEECH_BASE_URL`: Simba TTS for synthesis and Swivuriso for recognition. The Railway image sets `SPEECH_STT_ENABLED=false`, retains only Simba TTS, and delegates recognition to Groq to reduce memory and image size. The service is CPU-portable and lazy-loads its free Hugging Face model; generated isiXhosa MP3s are content-addressed and cached when `TTS_CACHE_ENABLED=true`.
...
Default Groq STT model: `whisper-large-v3` via `GROQ_STT_MODEL`.
